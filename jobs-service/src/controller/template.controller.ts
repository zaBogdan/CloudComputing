...
export default async function templateController(fastify: FastifyInstance) {
  ...
  fastify.get("/", async function (
    ...
    try {
      ...
      const response = await templateService.findAll({ owner: getCurrentUser(_request) }, queryParams.skip, queryParams.limit);
  ...
  fastify.get("/filter", async function (
    _request: FastifyRequest,
    reply: FastifyReply
  ) {
    try {
  ...
  fastify.post("/",{
    ...
    try {
      const { name, tags, parameters, substitute } = (_request.body as CreateTemplateSchema);
      const response = await templateService.createTemplate(name, tags, parameters, substitute, getCurrentUser(_request));
      reply.code(200).send(handleResponseSuccess(
  ...
  fastify.put("/:templateId",{
    ...
    try {
      ...
      const { name, tags, parameters, substitute } = (_request.body as UpdateTemplateSchema);
      const response = await templateService.updateTemplate(templateId, getCurrentUser(_request), name, tags, parameters, substitute);
      reply.code(200).send(handleResponseSuccess(
        'Successfully fetched all templates',
        response
      ));
  ...
  fastify.delete("/:templateId", async function (
    _request: FastifyRequest,
    reply: FastifyReply
  ) {
    try {
      ...
      await templateService.deleteTemplate(templateId, getCurrentUser(_request));
      reply.code(200).send(handleResponseSuccess(
